							...
							<el-row :gutter="20">
								<el-col :span="7">
									<el-form-item label="交易类型">
										<el-select v-model="tread_type_value" class="m-2" placeholder="Select" clearable style="width: 100%">
											<el-option v-for="item in tread_type_options" :key="item.value" :label="item.label" :value="item.value" />
										</el-select>
									</el-form-item>
								</el-col>
								<el-col :span="12">
									<el-form-item label="交易对">
										<el-select v-model="pairs_value" @change="handleChange" filterable placeholder="Select" clearable style="width: 100%" suffix-icon="Star">
											<!-- prefix 插槽 -->
											<template #prefix>
												<img v-if="selectedItem" :src="selectedItem.icon_url" style="width: 20px; height: 20px; margin-right: 10px" />
											</template>

											<el-option v-for="item in pairs_options" :key="item.value" :label="item.label" :value="item.value" style="display: flex; text-align: center; align-items: center">
												<img :src="item.icon_url" style="width: 20px; height: 20px; margin-right: 10px" />
												{{ item.label }}
					...
					<el-tabs v-model="activeName" class="demo-tabs" style="margin-top: 10px">
						<el-tab-pane label="账户详情" name="first">
							...
							<el-descriptions :column="1" style="margin-bottom: 0; padding: 0">
								<el-descriptions-item label="现货" align="center">
									<el-tag size="large">{{ user_spot_total_usdt }}</el-tag>
									<el-tag :type="现货盈亏tag颜色" size="large" style="margin-left: 5px">{{ 现货总盈亏 }}</el-tag>
								</el-descriptions-item>
							</el-descriptions>
							<el-table :data="user_spot_info_table_data" style="width: 100%">
								...
								<el-table-column label="相对0点浮动盈亏" align="center">
									<template #default="scope">
										<el-tag :type="scope.row.type" size="large">{{ scope.row.盈亏 }}</el-tag>
									</template>
								</el-table-column>
							</el-table>

							<el-descriptions :column="1" style="margin-top: 30px">
								<el-descriptions-item label="U本位合约" align="center">
									<el-tag size="large">{{ user_perp_total_usdt }}</el-tag>
									<el-tag :type="合约盈亏tag颜色" size="large" style="margin-left: 5px">{{ 合约总盈亏 }}</el-tag>
								</el-descriptions-item>
							</el-descriptions>
							<el-table :data="user_perp_info_table_data" style="width: 100%">
								...
								<el-table-column label="未实现盈亏" align="center">
									<template #default="scope">
										<el-tag :type="scope.row.type" size="large">{{ scope.row.unrealizedProfit }}</el-tag>
									</template>
								</el-table-column>
...
<script setup>
// 引入所需的库和方法
import { api_获取今日的统计记录, api_获取所有交易对, api_获取用户持仓 } from '@/api/binance_api'
import { fapi_获取用户持仓 } from '@/api/binance_fapi'
import { api_add_md_bot, api_get_md_bot, api_update_md_bot } from '@/api/md_bots'
