    ...
    <el-container>
        <el-main>
            ...
            <el-table ref="singleTableRef" :data="smading_strategy_list" style="width: 100%" :fit="false" border
                            ...
                            <el-row :gutter="20" style="margin-left: 60px; margin-top: 5px">
                                <el-col :span="6">
                                    ...
                                    <el-form-item label="币种" required>
                                        <el-select-v2 :options="symbol_options" placeholder="请选择"
                                            @change="updateSymbolPrecisionFields" style="width: 100%" clearable
                                            filterable value-key="symbol" />
                                        <el-button type="primary" size="small"
                                            @click="selectStartSymbolStrategy(props.row)"
                                            effect="dark">新增并启动</el-button>
                                    </el-form-item>
                                    ...
                                    <el-button type="primary" size="small"
                                        @click="selectContinueSymbolStrategy(props.row)" effect="dark">选中恢复</el-button>
                ...
                <el-table-column type="index" width="55" label="序号" align="center" />
                <el-table-column prop="exchange_type" label="交易所" width="100" show-overflow-tooltip sortable
                    align="center"></el-table-column>
                <el-table-column prop="exchange_name" label="交易所账号" width="130" show-overflow-tooltip sortable
                    align="center"></el-table-column>
                ...
                <el-table-column prop="pos_value_1st" label="首单价值" width="85" show-overflow-tooltip align="center"
                    v-if="show"></el-table-column>
                <el-table-column prop="cover_order_pos_value_1st" label="补单首单" width="85" show-overflow-tooltip
                    align="center" v-if="show"></el-table-column>
                ...
                <el-table-column fixed="right" prop="exchange_name" label="交易所账号" width="130" show-overflow-tooltip
            ...
            <el-dialog v-model="dialogVisible" :title="dialogTitle" width="65%" :before-close="handleClose"
                :close-on-click-modal="false" @closed="关闭策略明细弹窗()">
                <el-form :model="currentStrategy" label-width="150px">
                    <div class="dialog-content">
                        <el-card class="box-card"
                            ...
                            <el-row :gutter="20">
                                <el-col :span="12">
                                    <el-form-item label="交易所" required>
                                        <el-select v-model="exchange_type" clearable placeholder="请选择"
                                            style="width: 100%" filterable value-key="id"
                                            :disabled="currentStrategy.is_run" @change="更新交易所信息()">
                                            <el-option v-for="item in buzz_exchange_options" :key="item" :label="item"
                                                :value="item" />
                                        </el-select>
                                    </el-form-item>
                                </el-col>
                                <el-col :span="12">
                                    <el-form-item label="交易所账号" required>
                                        <el-select v-model="exchange_info" clearable placeholder="请选择"
                                            style="width: 100%" filterable value-key="id"
                                ...
                                <el-col :span="18">
                                    <div>
                                        <el-form-item label="主马丁持仓方向" required>
                                            <el-radio-group v-model="currentStrategy.position_side"
                                                class="my-radio-group" :disabled="currentStrategy.is_run">
                            ...
                            <!-- 交易对的多选框 -->
                            <el-form-item label="币种" required>
                                <el-select-v2 v-model="currentStrategy.symbols" :options="symbol_options"
                                    placeholder="请选择" @change="updateSymbolPrecisionFields" style="width: 100%" multiple
                                    clearable filterable value-key="symbol" />
                  ...
                  <el-option v-for="item in symbol_options" :key="item.symbol" :label="item.symbol"
                    :value="item.symbol" />
                        ...
                        <el-card class="box-card" style="margin-right: 20px; margin-left: 20px"
                            :disabled="currentStrategy.is_run" v-if="show">
                            <template #header>
                                <div class="card-header">
                                    <span>下单设置</span>
                                </div>
                                ...
                                <el-col :span="12">
                                    <el-form-item label="活跃订单数量" required>
                                        <el-input :disabled="currentStrategy.is_run"
                                            v-model.number="currentStrategy.live_order_num" placeholder="一次性挂多少个补单">
                            ...
                            <el-row :gutter="20">
                                ...
                                <el-col :span="12">
                                    <el-form-item label="补单波动" required>
                                        <el-input :disabled="currentStrategy.is_run"
                                            v-model.number="currentStrategy.price_pct_trigger_cover" type="number">
                                ...
                                <el-col :span="12">
                                    <div>
                                        <el-form-item label="叠加补单" required>
                                            <el-radio-group :disabled="currentStrategy.is_run"
                                                v-model.number="currentStrategy.open_add_cover" class="my-radio-group">
                        ...
                        <el-card class="box-card"
                            ...
                            v-if="show && currentStrategy.trade_type === 'futures'">
                            <template #header>
                                <div class="card-header">
                                    <span>对冲马丁设置</span>
                                </div>
                        ...
                        <el-card class="box-card"
                                ...
                                <el-col :span="12">
                                    <div>
                                        <el-form-item label="高单重置止盈" required>
                                            <el-radio-group
                                                v-model.number="currentStrategy.open_high_order_reset_take_profit"
                                                :disabled="currentStrategy.is_run" class="my-radio-group">
                                                <el-radio-button :label="true" class="my-radio-50">
                                                    <template #default>开启</template>
                                                </el-radio-button>
                                                <el-radio-button :label="false" class="my-radio-50">
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_high_order_reset_take_profit">
                                    <el-form-item label="第几单后重挂止盈" required>
                                        <el-input type="number"
                                            v-model.number="currentStrategy.when_repull_take_profit"></el-input>
                                    </el-form-item>
                                </el-col>
                                <el-col :span="12" v-if="currentStrategy.open_high_order_reset_take_profit">
                            ...
                            <el-row :gutter="20"
                                v-if="currentStrategy.open_take_profit && currentStrategy.position_side === 'BOTH'">
                                <el-col :span="12">
                                    <div>
                                        <el-form-item label="横盘重挂止盈" required>
                                            <el-radio-group
                                                v-model.number="currentStrategy.open_hp_order_reset_take_profit"
                                                :disabled="currentStrategy.is_run" class="my-radio-group">
                                                <el-radio-button :label="true" class="my-radio-50">
                                                    <template #default>开启</template>
                                                </el-radio-button>
                            ...
                            <el-row :gutter="20"
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_hp_order_reset_take_profit">
                                    <el-form-item label="0本轮启动" required>
                                        ...
                                        <el-input type="number"
                                            v-model.number="currentStrategy.when_trigger_hg_order_reset_take_profit">
                            ...
                            <el-row :gutter="20"
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_hp_order_reset_take_profit">
                                    <el-form-item label="0第" required>
                                        <el-input type="number"
                                            v-model.number="currentStrategy.when_hg_repull_take_profit">
                            ...
                            <el-row :gutter="20"
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_hp_order_reset_take_profit">
                                    <el-form-item label="1本轮启动" required>
                                        <el-input type="number"
                                            v-model.number="currentStrategy.when_trigger_hg_order_reset_take_profit1">
                                            <template #append>分钟后触发</template>
                                        </el-input>
                                    </el-form-item>
                                </el-col>
                            ...
                            <el-row :gutter="20"
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_hp_order_reset_take_profit">
                                    <el-form-item label="1第" required>
                                        <el-input type="number"
                                            v-model.number="currentStrategy.when_hg_repull_take_profit1">
                                            <template #append>单后横盘重挂止盈</template>
                                        </el-input>
                                    </el-form-item>
                                </el-col>
                            ...
                            <el-row :gutter="20"
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_hp_order_reset_take_profit">
                                    <el-form-item label="2本轮启动" required>
                                        <el-input type="number"
                                            v-model.number="currentStrategy.when_trigger_hg_order_reset_take_profit2">
                                            <template #append>分钟后触发</template>
                                        </el-input>
                                    </el-form-item>
                                </el-col>
                            ...
                            <el-row :gutter="20"
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_hp_order_reset_take_profit">
                                    <el-form-item label="2第" required>
                                        <el-input type="number"
                                            v-model.number="currentStrategy.when_hg_repull_take_profit2">
                                            <template #append>单后横盘重挂止盈</template>
                                        </el-input>
                                    </el-form-item>
                                </el-col>
                            ...
                            <el-row :gutter="20">
                                <el-col :span="12"
                                    v-if="currentStrategy.open_take_profit && currentStrategy.trade_type === 'futures'">
                                    <div>
                            ...
                            <el-row :gutter="20" v-if="!currentStrategy.open_stop_profit_switch_hedge_mading">
                                ...
                                <el-col :span="12" v-if="currentStrategy.open_stop_profit">
                                    <el-form-item label="止损前等待时间" required>
                                        <el-input type="number"
                                            v-model.number="currentStrategy.before_stop_profit_wait_time"
                                            :disabled="currentStrategy.is_run">
                                ...
                                <el-col :span="12">
                                    <el-form-item label="补到多少单微信告警">
                                        <el-input type="number" v-model.number="currentStrategy.cover_alarm_num"
                                            :disabled="currentStrategy.is_run" placeholder="比如第20补单挂上后触发告警"></el-input>
                                    </el-form-item>
                                </el-col>
                                <el-col :span="12">
                                    <el-form-item label="第几单暂停补单">
                                        ...
                                        <el-input type="number" v-model.number="currentStrategy.cover_order_pause_num"
                                            :disabled="currentStrategy.is_run" placeholder=""></el-input>
                                    </el-form-item>
                                </el-col>
                            </el-row>
                            <el-row>
                                ...
                                <el-col :span="12">
                                    <div>
                                        <el-form-item label="触发后微信告警" required>
                                            <el-radio-group :disabled="currentStrategy.is_run"
                                                v-model.number="currentStrategy.open_falls_tigger_wx_alarm"
                                                class="my-radio-group">
                                                <el-radio-button :label="true" class="my-radio-50">
                                                    <template #default>开启</template>
                                                </el-radio-button>
            ...
            <el-dialog v-model="copyDialogVisible" title="复制交易对信息" width="35%" :before-close="copyHandleClose"
                ...
                <el-select-v2 v-model="currentStrategy.symbols" :options="symbol_options" placeholder="请选择"
                    @change="updateSymbolPrecisionFields" style="width: 100%" multiple clearable filterable
                    value-key="symbol" />
                <div slot="footer" class="dialog-footer" style="margin-top: 20px">
            ...
            <el-dialog v-model="mockDialogVisible" title="模拟数据" width="65%" :before-close="mockHandleClose"
                ...
                <el-card class="box-card">
                    ...
                    <el-table :data="mock_long_table_list" style="width: 100%" border highlight-current-row
                        height="800px" stripe>
                ...
                <el-card class="box-card" style="margin-top: 20px"
                    ...
                    <el-table :data="mock_short_table_list" style="width: 100%" border highlight-current-row
                        height="800px" stripe>
                        <el-table-column prop="当前补单次数" label="当前补单次数" width="120" show-overflow-tooltip
                            align="center"></el-table-column>
                        <el-table-column prop="补单数量" label="补单数量" width="100" show-overflow-tooltip
