    ...
    <el-main>
      <el-row :gutter="20" style="margin-bottom: 20px">
        <el-col :span="6">
          ...
          <el-button type="primary" @click="copyStrategy()"
            >复制选中策略</el-button
          >
          <el-button type="primary" @click="copySymbolsInfo()"
            >复制币种信息</el-button
          >
        </el-col>
            ...
            <el-row :gutter="20" style="margin-top: 10px">
              <el-col :span="12">
                <el-form-item label="是否禁用">
                  <el-select
                    @change="change_is_deleted"
                    v-model="strategy_is_deleted"
                    clearable
                    placeholder="请选择"
                    style="width: 100%"
                  >
              ...
              <el-col :span="2">
                <el-button
                  type="primary"
                  @click="getStartegyList(strategy_is_deleted)"
                >
                  查询
                </el-button>
              </el-col>
      ...
      <el-table
        ref="singleTableRef"
        :data="smading_strategy_list"
        style="width: 100%"
        :fit="false"
        border
        highlight-current-row
        @current-change="handleSelectionChangeOuter"
      ...
      >
        <el-table-column type="expand">
          <template #default="props">
              ...
              <el-row :gutter="20" style="margin-left: 60px; margin-top: 5px">
                <el-col :span="6">
                  <el-button
                    type="primary"
                    size="small"
                    @click="selectStartSymbolStrategy(props.row)"
                    effect="dark"
                  >
                    选中启动
                  </el-button>
                  <el-button
                    type="primary"
                    size="small"
                    @click="selectPauseSymbolStrategy(props.row)"
                    effect="dark"
                  >
                    选中暂停
                  </el-button>
                  <el-button
                    type="primary"
                    size="small"
                    @click="selectContinueSymbolStrategy(props.row)"
                    effect="dark"
                  >
                    选中恢复
                  </el-button>
                  <el-button
                    type="primary"
                    size="small"
                    @click="selectStopSymbolStrategy(props.row)"
                    effect="dark"
                  >
                    选中停止
                  </el-button>
                  <el-button
                    type="danger"
                    size="small"
                    @click="selectDeleteSymbolStrategy(props.row)"
                    effect="dark"
                  >
                    选中删除
                  </el-button>
            ...
            <el-table
              :data="props.row.symbol_infos"
              style="margin-left: 50px; width: 80%"
              @selection-change="
                (selectedRows) =>
                  handleSelectionChangeInner(selectedRows, props.row.id)
              "
            >
              ...
              >
                <template #default="{ row }">
                  <el-tag
                    :type="row.is_run ? 'success' : 'danger'"
                    effect="dark"
                  >
                    {{ row.is_run ? "是" : "否" }}
                  </el-tag>
              ...
              >
                <template #default="{ row }">
                  <el-tag
                    :type="!row.is_pause ? 'success' : 'danger'"
                    effect="dark"
                  >
                    {{ row.is_pause ? "是" : "否" }}
                  </el-tag>
              ...
              <el-table-column label="操作" width="380" align="center">
                <template #default="{ row }">
                  ...
                  <el-button
                    type="primary"
                    size="small"
                    @click="pauseSymbolStrategy(row)"
                    plain
                    :disabled="row.is_pause"
                  >
                    暂停
                  </el-button>
                  <el-button
                    type="primary"
                    size="small"
                    @click="continueSymbolStrategy(row)"
                    plain
                    :disabled="!row.is_pause"
                  >
                    恢复
                  ...
                  <el-button
                    type="danger"
                    size="small"
                    @click="deleteSymbolStrategy(row)"
                    :disabled="row.is_run"
                  >
                    删除
                  </el-button>
        ...
        ></el-table-column>
        <el-table-column
          prop="cover_order_pos_value_1st"
          label="补单首单"
          width="85"
          show-overflow-tooltip
          align="center"
          ...
          <div class="dialog-content">
            ...
            >
              ...
              <el-form-item label="策略名称(备注)" required>
                <el-input
                  v-model="currentStrategy.strategy_note"
                  autosize
                  type="textarea"
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="交易所" required>
                    ...
                    >
                      <el-option
                        v-for="item in buzz_exchange_options"
                        :key="item.id"
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="交易类型" required>
                    <el-radio-group
                      :disabled="currentStrategy.is_run"
                      v-model="currentStrategy.trade_type"
                ...
                <el-col :span="18">
                  <div>
                    <el-form-item label="主马丁持仓方向" required>
                      <el-radio-group
                        v-model="currentStrategy.position_side"
                        class="my-radio-group"
                        :disabled="currentStrategy.is_run"
                      >
              ...
              <el-form-item label="币种" required>
                <el-select
                  v-model="currentStrategy.symbols"
                  @change="updateSymbolPrecisionFields"
                  clearable
                  placeholder="请选择"
                  style="width: 100%"
                  multiple
              ...
              <!-- 动态的交易对精度编辑框 -->
              <el-row
                v-for="(precision, symbol) in symbol_precisions"
                :key="symbol"
                :gutter="20"
              >
                <el-col :span="13">
                  ...
                  >
                    <el-input
                      v-model.number="symbol_precisions[symbol]"
                      placeholder="比如14.0125就填4，代表4位数"
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="活跃订单数量" required>
                    <el-input
                      :disabled="currentStrategy.is_run"
                      v-model.number="currentStrategy.live_order_num"
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="补单次数" required>
                    <el-input
                      :disabled="currentStrategy.is_run"
                      v-model.number="currentStrategy.all_cover_order_count"
                    >
                ...
                <el-col :span="12">
                  <el-form-item label="补单波动" required>
                    <el-input
                      :disabled="currentStrategy.is_run"
                      v-model.number="currentStrategy.price_pct_trigger_cover"
                      type="number"
                    >
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <div>
                    <el-form-item label="叠加补单" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="currentStrategy.open_add_cover"
                        class="my-radio-group"
                      >
                ...
                <el-col :span="12">
                  <el-form-item
                    label="叠加补单首单价值"
                    required
                    v-if="currentStrategy.open_add_cover"
                  >
                    <el-input
                      type="number"
                      :disabled="currentStrategy.is_run"
                      v-model.number="
                        currentStrategy.add_cover_order_pos_value_1st
                      "
                    >
                      <template #append>USDT</template>
                    </el-input>
                ...
                <el-col :span="12">
                  <el-form-item label="叠加补单波动" required>
                    <el-input
                      :disabled="currentStrategy.is_run"
                      v-model.number="
                        currentStrategy.add_price_pct_trigger_cover
                      "
                      type="number"
                    >
                      <template #append>%</template>
                ...
                <el-col :span="12">
                  <el-form-item label="叠加加速补单百分比" required>
                    <el-input
                      :disabled="currentStrategy.is_run"
                      type="number"
                      v-model.number="currentStrategy.add_accel_cover_pct"
                      placeholder="正数加快补单负数放慢补单"
                    >
                      <template #append>%</template>
                    </el-input>
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="补到多少单停止" required>
                    <el-input
                      type="number"
                      v-model.number="currentStrategy.cover_stop_num"
                      placeholder="比如第30补单挂上后停止挂补单.不填则表示全补"
                    ></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            ...
            >
                ...
                <el-col :span="12">
                  <div>
                    <el-form-item label="对冲马丁" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="currentStrategy.open_hedge_mading"
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                          <template #default>开启</template>
                ...
                <el-col :span="12">
                  <el-form-item label="对冲马丁首单" required>
                    <el-input
                      ...
                      placeholder="如果想补单第n单补进后就挂对冲首单就填 n+1"
                    ></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="对冲马丁首单价值" required>
                    <el-input
                      ...
                      type="number"
                      v-model.number="
                        currentStrategy.hedge_mading_pos_value_1st
                      "
                    >
                      <template #append>USDT</template>
                    </el-input>
                  ...
                  <div>
                    <el-form-item label="对冲触发后停止补单" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="
                          currentStrategy.open_tigger_hedge_mading_stop_cover
                        "
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
              ...
              <el-row
                :gutter="20"
                v-if="
                  currentStrategy.open_hedge_mading &&
                  !currentStrategy.open_tigger_hedge_mading_stop_cover
                "
              >
                <el-col :span="12">
                  <el-form-item label="对冲马丁第几单补单" required>
                    <el-input
                      :disabled="currentStrategy.is_run"
                      v-model.number="
                        currentStrategy.tigger_hedge_mading_cover_num
                      "
                      placeholder="补单第n单补进后对冲开始补单就填 n+1(注意补单必须大于首单 不然不会补)"
                    ></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="补单首单价值" required>
                    <el-input
                      ...
                      :disabled="currentStrategy.is_run"
                      v-model.number="
                        currentStrategy.hedge_mading_cover_pos_value_1st
                      "
                    >
                      <template #append>USDT</template>
                    </el-input>
            ...
            >
                ...
                <el-col :span="12">
                  <div>
                    <el-form-item label="止盈" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="currentStrategy.open_take_profit"
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                          <template #default>开启</template>
              ...
              <el-row :gutter="20">
                <el-col :span="12" v-if="currentStrategy.open_take_profit">
                  <div>
                    <el-form-item label="止盈方式" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model="currentStrategy.take_profit_type"
                        class="my-radio-group"
                      >
                ...
                <el-col
                  :span="12"
                  v-if="
                    currentStrategy.open_take_profit &&
                    (currentStrategy.take_profit_type === 'price' ||
                      currentStrategy.take_profit_type === 'mixed')
                  "
                >
                ...
                <el-col
                  ...
                  v-if="
                    currentStrategy.open_take_profit &&
                    (currentStrategy.take_profit_type === 'percent' ||
                      currentStrategy.take_profit_type === 'mixed')
                  "
                >
                ...
                <el-col :span="12">
                  <div>
                    <el-form-item label="高单重置止盈" required>
                      <el-radio-group
                        v-model.number="
                          currentStrategy.open_high_order_reset_take_profit
                        "
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
              ...
              <el-row :gutter="20">
                <el-col
                  :span="12"
                  v-if="currentStrategy.open_high_order_reset_take_profit"
                >
                  <el-form-item label="第几单后重挂止盈" required>
                    <el-input
                      type="number"
                      v-model.number="currentStrategy.when_repull_take_profit"
                    ></el-input>
                  </el-form-item>
                </el-col>
                <el-col
                ...
                >
                  <el-form-item label="重挂止盈大小" required>
                    <el-input
                      type="number"
                      v-model="currentStrategy.repull_take_profit_num"
              ...
              <el-row
                ...
                v-if="
                  currentStrategy.open_take_profit &&
                  currentStrategy.position_side === 'BOTH'
                "
              >
                <el-col :span="12">
                  <div>
                    <el-form-item label="横盘重挂止盈" required>
                      <el-radio-group
                        v-model.number="
                          currentStrategy.open_hp_order_reset_take_profit
                        "
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
              ...
              >
                <el-col
                  :span="12"
                  v-if="currentStrategy.open_hp_order_reset_take_profit"
                >
                  <el-form-item label="0本轮启动" required>
                    <template #label
                      ...
                      >
                        <template #content>
                          横盘重挂止盈可以设置3组,0为第一组 1为第二组
                          2为第三组。比如:<br />
                          第一组设置>=30分钟且补单>=15单，则保本跑就设置30 15
                          0<br />
                          第二组设置>=60分钟且补单>=18单，则亏1u跑,设置为60 18
                          -1<br />
                          第三组设置>=90分钟且补单>=20单，则亏2u跑,设置为90 20
                          -2<br />
                          可以3组都设置也可以只设置1组。<br />
                        ...
                        <el-icon
                          :span="12"
                          size="20"
                          style="margin-left: 2px; margin-top: 6px"
                        >
                          <QuestionFilled />
                        </el-icon>
                      </el-tooltip>
                    ...
                    <el-input
                      type="number"
                      v-model.number="
                        currentStrategy.when_trigger_hg_order_reset_take_profit
                      "
                      ><template #append>分钟后触发</template></el-input
                    >
                  </el-form-item>
                </el-col>
              </el-row>
              ...
              >
                ...
                >
                  <el-form-item label="0第" required>
                    <el-input
                      type="number"
                      v-model.number="
                        currentStrategy.when_hg_repull_take_profit
                      "
                      ><template #append>单后横盘重挂止盈</template></el-input
                    >
                  </el-form-item>
                </el-col>
                <el-col
                ...
                >
                  <el-form-item label="0横盘重挂止盈大小" required>
                    <el-input
                      type="number"
                      v-model="currentStrategy.hg_repull_take_profit_num"
                      @change="handle_repull_take_profit_num"
                    >
                      <template #append>USDT</template>
                    </el-input>
                ...
                >
                  <el-form-item label="1本轮启动" required>
                    <el-input
                      type="number"
                      v-model.number="
                        currentStrategy.when_trigger_hg_order_reset_take_profit1
                      "
                      ><template #append>分钟后触发</template></el-input
                    >
                  </el-form-item>
              ...
              >
                ...
                >
                  <el-form-item label="1第" required>
                    <el-input
                      type="number"
                      v-model.number="
                        currentStrategy.when_hg_repull_take_profit1
                      "
                      ><template #append>单后横盘重挂止盈</template></el-input
                    >
                  </el-form-item>
                ...
                >
                  <el-form-item label="1横盘重挂止盈大小" required>
                    <el-input
                      type="number"
                      v-model="currentStrategy.hg_repull_take_profit_num1"
                      @change="handle_repull_take_profit_num"
                    >
                      <template #append>USDT</template>
                    </el-input>
                ...
                >
                  <el-form-item label="2本轮启动" required>
                    <el-input
                      type="number"
                      v-model.number="
                        currentStrategy.when_trigger_hg_order_reset_take_profit2
                      "
                      ><template #append>分钟后触发</template></el-input
                    >
                  </el-form-item>
              ...
              >
                ...
                >
                  <el-form-item label="2第" required>
                    <el-input
                      type="number"
                      v-model.number="
                        currentStrategy.when_hg_repull_take_profit2
                      "
                      ><template #append>单后横盘重挂止盈</template></el-input
                    >
                  </el-form-item>
                ...
                >
                  <el-form-item label="2横盘重挂止盈大小" required>
                    <el-input
                      type="number"
                      v-model="currentStrategy.hg_repull_take_profit_num2"
                      @change="handle_repull_take_profit_num"
                    >
                      <template #append>USDT</template>
                    </el-input>
              ...
              <el-row :gutter="20">
                <el-col
                  ...
                  v-if="
                    currentStrategy.open_take_profit &&
                    currentStrategy.trade_type === 'futures'
                  "
                >
                  <div>
                    <el-form-item label="同时止盈" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model="currentStrategy.take_profit_together"
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                          <template #default>开启</template>
              ...
              >
                ...
                <el-col
                  :span="12"
                  v-if="currentStrategy.open_float_take_profit"
                >
              ...
              >
                  ...
                  <div>
                    <el-form-item label="浮盈止盈价格回撤" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="
                          currentStrategy.open_float_take_profit_back
                        "
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                ...
                <el-col
                  :span="12"
                  v-if="currentStrategy.open_float_take_profit_back"
                >
                  <el-form-item label="浮盈止盈回撤价格" required>
                    <el-input
                      ...
                      type="number"
                      v-model.number="
                        currentStrategy.float_take_profit_back_price
                      "
                    >
                      <template #append>USDT</template>
                    </el-input>
            ...
            >
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <div>
                    <el-form-item label="开启止损" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="currentStrategy.open_stop_profit"
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                          <template #default>开启</template>
                ...
                <el-col
                  :span="12"
                  v-if="
                    currentStrategy.open_stop_profit &&
                    !currentStrategy.open_stop_profit_switch_hedge_mading
                  "
                >
                  <el-form-item label="止损后等待时间" required>
                    <el-input
                      type="number"
                      v-model.number="currentStrategy.stop_profit_wait_time"
                    >
              ...
              >
                ...
                <el-col
                  :span="12"
                  v-if="currentStrategy.open_stop_profit_switch_hedge_mading"
                >
                  <el-form-item label="对冲马丁策略" required>
                    <el-select
                      v-model="对冲马丁策略id"
                      @change="当前策略对应的对冲马丁的策略改变;"
                      clearable
                      placeholder="请选择对应策略的序号"
                      style="width: 100%"
                      filterable
                    >
                      <el-option
                        v-for="item in 当前策略对应的对冲马丁的策略列表"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      />
                ...
                <el-col :span="12" v-if="currentStrategy.open_stop_profit">
                  <div>
                    <el-form-item label="止损后自动暂停" required>
                      <el-radio-group
                        v-model.number="
                          currentStrategy.after_stop_profit_auto_pause
                        "
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                ...
                <el-col :span="12" v-if="currentStrategy.open_stop_profit">
                  <el-form-item label="止损前等待时间" required>
                    <el-input
                      type="number"
                      v-model.number="
                        currentStrategy.before_stop_profit_wait_time
                      "
                    >
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="补到多少单微信告警">
                    <el-input
                      type="number"
                      v-model.number="currentStrategy.cover_alarm_num"
                      placeholder="比如第20补单挂上后触发告警"
                    ></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="第几单暂停补单">
                    <template #label
                      ...
                      >
                        <template #content>
                          填数字。不填就不启用该功能，此功能一轮马丁只触发一次，止盈后自动恢复
                        </template>
                        <el-icon
                          :span="12"
                          size="20"
                    ...
                    <el-input
                      type="number"
                      v-model.number="currentStrategy.cover_order_pause_num"
                      placeholder=""
                    ></el-input>
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="12">
                  <div>
                    <el-form-item required>
                      ...
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="currentStrategy.open_prevent_falls"
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                          <template #default>开启</template>
              ...
              <el-row :gutter="2" v-if="currentStrategy.open_prevent_falls">
                <el-col :span="8">
                  <el-form-item label="触发条件">
                    <el-input
                      type="number"
                      v-model.number="currentStrategy.falls_second_num"
                    >
                ...
                <el-col :span="8">
                  <el-input
                    type="number"
                    v-model.number="currentStrategy.falls_cover_num"
                  >
                ...
                <el-col :span="12">
                  <div>
                    <el-form-item label="触发后暂停补单" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="currentStrategy.open_falls_tigger_pause"
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
                          <template #default>开启</template>
                  ...
                  <div>
                    <el-form-item label="触发后微信告警" required>
                      <el-radio-group
                        :disabled="currentStrategy.is_run"
                        v-model.number="
                          currentStrategy.open_falls_tigger_wx_alarm
                        "
                        class="my-radio-group"
                      >
                        <el-radio-button :label="true" class="my-radio-50">
      ...
      >
        ...
        >
          <el-option
            v-for="item in strategy_index_options"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          />
        ...
        <div slot="footer" class="dialog-footer" style="margin-top: 20px">
          ...
          <el-button type="primary" @click="submitCopySymbolStrategy"
            >确定</el-button
          >
        </div>
      </el-dialog>
      ...
      <el-dialog
        ...
        title="模拟数据"
        width="65%"
        :before-close="mockHandleClose"
        :close-on-click-modal="false"
        v-if="mock_positon_side === 'both' || mock_positon_side === 'long'"
      >
        ...
        <div slot="footer" class="dialog-footer" style="margin-top: 20px">
          <el-button type="primary" @click="mockDialogVisible = false"
            >关闭</el-button
          >
        </div>
      </el-dialog>
...
const submitCopySymbolStrategy = async () => {
  ...
  try {
    const res = await api_复制交易对信息(
      target_copy_id.value,
      currentStrategy.value.symbol_infos
    );
    // console.log("res", res);
...
const copyStrategy = () => {
  ...
  currentStrategy.value.symbol_infos.forEach((symbol_info) => {
    symbol_precisions[symbol_info["symbol"]] =
      symbol_info["symbol_price_precision"];
...
watch(
  ...
  (val) => {
    ...
    if (
      currentStrategy.value.cover_stop_num &&
      currentStrategy.value.all_cover_order_count
    ) {
      let max = Math.min(
        currentStrategy.value.cover_stop_num,
        currentStrategy.value.all_cover_order_count
...
function editStrategy(item) {
  ...
  if (
    currentStrategy.value.symbol_infos &&
    currentStrategy.value.symbol_infos.length > 0
  ) {
    currentStrategy.value.symbol_infos.forEach((symbol_info) => {
      symbol_precisions[symbol_info["symbol"]] =
        symbol_info["symbol_price_precision"];
...
const 禁用策略 = async (row, tag) => {
  ...
  try {
    const res = await api_更新指定id的双马丁策略(
...
const 模拟数据 = async () => {
  ...
  try {
    ...
    if (res.status === 200 && res.data.code === 200) {
      ...
      if (
        res.data.data.hasOwnProperty("long") &&
        res.data.data.hasOwnProperty("short")
      ) {
